...
# msgids are generated by mesh-send (timestamp + hash); reject anything outside
# that charset so a stray argument can't reach the filesystem as a path
if [ -n "$TARGET" ] && ! printf '%s' "$TARGET" | grep -qE '^[A-Za-z0-9_-]+$'; then
  echo "Error: invalid msgid '$TARGET'" >&2
  exit 1
fi
